angular.module('QuestionCtrl', [])
        .controller('QuestionController', ['$timeout', '$scope', '$cookieStore', '$rootScope', '$location', '$state', '$http', '$q', '$stateParams', 'flash', '$modal', 'appAlert', 'Answer', 'Question', 'socket', 'Notifi', function ($timeout, $scope, $cookieStore, $rootScope, $location, $state, $http, $q, $stateParams, flash, $modal, appAlert, Answer, Question, socket, Notifi) {
                ...
                $scope.formData.selectedCity = $scope.listCity[0];
                ...
                $scope.createQuestion = function () {
                    ...
                    Question.create({
                        ...
                        district: $scope.formData.selectedDistrict,
                        city: $scope.formData.selectedCity,
                        tag: $scope.formData.selectedTag
                    })
                        .success(function (data) {
                            ...
                            $('.show-form').fadeOut();
                            if (!data.status) {
                                flash.success = 'Tin của bạn đã được gửi và đang chờ quản trị viên xét duyệt. Cám ơn bạn đã đăng bài.';
                                $state.go('home');
                            } else {
                                flash.success = 'Đăng Tin thành công!';
                ...
                $scope.createQuestion2 = function () {
                    $scope.Proccess = true;
                    Question.create({
                ...
                $scope.deleteQuestion = function (id, path) {
                    appAlert.confirm({
                        title: 'Xóa',
                        message: 'Bạn chắc chắn muốn xóa tin này ?'
                    }, function (isOk) {
                        if (isOk) {
                                        ...
                                        $http.get('api/admin').success(function (user) {
                                            if (user.indexOf(data.userId._id) != -1) {
                                                Notifi.create({
                                                    ...
                                                    content: 'Tin ' + data.title + ' đã bị quản trị xóa!'
                                                });
                            ...
                            Question.delete(id)
                                    ...
                                    .success(function (data) {
                                        socket.emit('new question');
                                        flash.success = 'Xóa tin thành công!';
                                        ...
                                        $http.get('api/question/detail/' + id)
                                                .success(function (data) {
                                                    Notifi.create({
                                                        userRecive: data.userId._id,
                                                        userSend: $cookieStore.get('currentUser')._id,
                                                        content: 'Tin ' + data.title + ' đã được quản trị đăng!',
                                                        questionId: id
                                                    });
                                                    socket.emit('approve', {
                                                        userTitle: data.title,
                ...
                $scope.closeForm = function () {
                    appAlert.confirm({
                        title: 'Xác nhận hủy',
                        message: 'Bạn chắc chắn muốn hủy đăng tin này ?'
                    }, function (isOk) {
                        if (isOk) {
                            $('.show-form').fadeOut(500);
                            $rootScope.suggestTag = null;
                                            ...
                                            .success(function (listFA) {
                                                for (var i in listFA) {
                                                    ...
                                                    Notifi.create({
                                                        ...
                                                        userSend: $cookieStore.get('currentUser')._id,
                                                        content: $cookieStore.get('currentUser').displayName + ' đã trả lời tin ' + data.title,
                                                        questionId: item._id
                                                    });
                    ...
                    if (!$.isEmptyObject($scope.questionData)) {
                        Question.edit($scope.questionData)
                                .success(function (data) {
                                    ...
                                    flash.success = 'Sửa tin thành công!';
                                });
                    } else {
                            ...
                            .success(function (data) {
                                if (data !== '0') {
                                    $http.get('api/question/vote_up/' + id)
                                            .success(function (data) {
                                                if (parseInt(data) == 1) {
                                                    flash.success = 'Bạn đã BỎ thích tin này!';
                                                } else {
                                                    $http.get('api/question/detail/' + id)
                                                            .success(function (data) {
                                                                if ($cookieStore.get('currentUser')._id != data.userId._id) {
                                                                    Notifi.create({
                                                                        userRecive: data.userId._id,
                                                                        userSend: $cookieStore.get('currentUser')._id,
                                                                        content: $cookieStore.get('currentUser').displayName + ' đã thích tin ' + data.title,
                                                                        questionId: id
                                                                    });
                                                                    socket.emit('voteup', {
                                                                        userSendName: $cookieStore.get('currentUser').displayName,
                                                    ...
                                                    flash.success = 'Bạn đã thích tin này!';
                                                }
                                                Question.get()
                    ...
                    $http.get('/loggedin').success(function (data) {
                        if (data !== '0') {
                            $http.get('api/question/vote_down/' + id)
                                    .success(function (data) {
                                        if (parseInt(data) == 1) {
                                            flash.success = 'Bạn đã bỎ không thích tin này!';
                                        } else {
                                            flash.success = 'Bạn không thích tin này!';
                                        }
                                        Question.get()
                                                .success(function (data) {
                                                    $scope.listQuestion = data;
                                    ...
                                    $http.get('api/question/favorite/' + id)
                                            .success(function (favorite) {
                                                ...
                                                if (parseInt(favorite) == 1) {
                                                    ...
                                                    $http.get('api/question/detail/' + id)
                                                            .success(function (data) {
                                                                Notifi.create({
                                                                    userRecive: data.userId._id,
                                                                    userSend: $cookieStore.get('currentUser')._id,
                                                                    content: $cookieStore.get('currentUser').displayName + ' đã theo dõi tin ' + data.title,
                                                                    questionId: id
                                                                });
                                                    ...
                                                    flash.success = 'Bạn đã theo dõi tin này!';
                                                }
                            ...
                            .success(function (data) {
                                if (data !== '0') {
                                    Question.checkReport(id)
                                            .success(function (data) {
                                                if (data != null) {
                                                    flash.error = 'Bạn đã báo cáo tin này vi phạm rồi!';
                                                } else {
                                                    appAlert.report({
                                                        title: 'Xác nhận',
                                                        message: 'Bạn chắc chắn muốn báo cáo tin này vi phạm?'
                                                    }, function (isOk) {
                                                        if (isOk) {
                                                            Question.report({
                                                                questionId: id,
                                                                                            ...
                                                                                            .success(function (user) {
                                                                                                for (var i in user) {
                                                                                                    ...
                                                                                                    Notifi.create({
                                                                                                        userRecive: item._id,
                                                                                                        userSend: $cookieStore.get('currentUser')._id,
                                                                                                        content: $cookieStore.get('currentUser').displayName + ' báo cáo vi phạm tin ' + data.title,
                                                                                                        questionId: id
                                                                                                    });
