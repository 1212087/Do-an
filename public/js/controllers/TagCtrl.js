...
.controller('getQuestionByTagController', ['$scope', '$stateParams', '$http', function($scope, $stateParams, $http) {
    ...
    var district = "Quận 1";
    $http.get('/api/question/getQuestionByDistrict/' + district)
        .success(function(data) {
            $scope.listQuestionByDis = data;
        })
        .error(function() {
            console.log('error');
        });
...
.controller('getQuestionByDistrictController', ['$scope', '$stateParams', '$http', function($scope, $stateParams, $http) {
    $scope.currentPage = 1;
    $scope.maxSize = 5;
    $scope.entryLimit = 10;
    var district = "quận 1";
