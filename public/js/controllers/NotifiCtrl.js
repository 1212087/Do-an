...
.controller('NotifiController', ['$scope', '$cookieStore', '$window', '$state', '$location', '$http', '$rootScope', 'appAlert', 'flash', 'AuthenticationService', 'socket', 'Notifi', function($scope, $cookieStore, $window, $state, $location, $http, $rootScope, appAlert, flash, AuthenticationService, socket, Notifi) {
    ...
    socket.on('voteup', function(data) {
        if ($cookieStore.get('currentUser')._id == data.userReciveId) {
            flash.success = '<a href="cau-hoi/chi-tiet/' + data.questionIds + '/' + '" >' + data.userSendName + ' đã thích tin ' + data.userTitle + '</a>';
        }
    ...
    socket.on('createAnswer', function(data) {
        if ($cookieStore.get('currentUser')._id == data.userReciveId) {
            flash.success = data.userSendName + ' đã trả lời tin ' + data.userTitle;
        }
            ...
            .success(function(listAM) {
                ...
                for (var i in listAD) {
                    ...
                    if ($cookieStore.get('currentUser')._id == item) {
                        flash.success = '<a href="cau-hoi/chi-tiet/' + data.questionIds + '/">' + data.userSendName + ' báo cáo vi phạm tin ' + data.userTitle + '</a>';
                    }
                }
            });
    ...
    socket.on('approve', function(data) {
        if ($cookieStore.get('currentUser')._id == data.userReciveId) {
            flash.success = '<a href="cau-hoi/chi-tiet/' + data.questionIds + '/">Tin ' + data.userTitle + ' đã được quản trị đăng!</a>';
        }
    ...
    socket.on('Favorite', function(data) {
        if ($cookieStore.get('currentUser')._id == data.userReciveId) {
            flash.success = '<a href="cau-hoi/chi-tiet/' + data.questionIds + '/">' + data.userSendName + ' đã theo dõi tin ' + data.userTitle + '</a>';
        }
    ...
    socket.on('deleteQuestion', function(data) {
        if ($cookieStore.get('currentUser')._id == data.userReciveId) {
            flash.error = 'Tin ' + data.userTitle + ' đã bị quản trị xóa!';
        }
